...
import type {ResultProps, UploadResultLBProps, ResultRefProps, ResultFileInfo} from "@luban/ui-upload";
import {clsx} from "@luban/style";
import {FormStoreContext, FormLayoutContext, FormGroupContext} from "../context";
import type {FormItemPropsType, FormItemRef} from "../interface";
...
const FormItem: FC<FormItemPropsType> = (props) => {
    const {
        name: realName = "",
        children,
    ...
    const [forceUpdateFlag, setForceUpdateFlag] = useState<number>(Date.now());
    const [error, setError] = useState<string>();
    const uploadResultRef = useRef<ResultRefProps>(null);
    const formItemRef = useRef<FormItemRef>(null);
    const defaultValueRef = useRef<unknown>(defaultVal);
    const formObjRef = useRef<{
        hide: boolean;
        label?: string;
        displayName?: string;
    }>({hide, label});
    ...
    const displayName = useMemo(() => {
        if (name !== "" && !Array.isArray(children) && children instanceof Object) {
            return (children as React.FunctionComponentElement<unknown>).type.displayName;
        }
        return undefined;
    }, [children, name]);

    const onChange = useCallback((initValue: unknown) => {
        // todo
        if (name !== "") {
    ...
    const onUploadRemove = useCallback((id: string | Promise<string | null>, isInit: boolean) => {
        if (isInit) {
            setFormValue({[name]: (getFormValue([name], true)[name] as ResultFileInfo[]).filter((item) => item.id !== id)});
        }
        if (children instanceof Object && "props" in children) {
            // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
    ...
    const uploadResults = useCallback(() => {
        if (children instanceof Object) {
            ...
            switch (displayName) {
                ...
                case "UploadImage":
                    return (
                        <div className={clsx(uploadResultContainer, resultMaxHeightStyle, !showUploadResults && hideUploadResult)} key={`${forceUpdateFlag}-result`}>
                            <UploadResults
                                // eslint-disable-next-line @typescript-eslint/consistent-type-assertions
                                {...{...uploadResult, initialFile: getFormValue([name], true)[name] ?? defaultVal} as ResultProps}
                                onRemoveFile={onUploadRemove}
                                ref={uploadResultRef}
                                disabled={disabled}
                                /** 兼容formList情况上传组件相同id的情况，使用name能够保证id的唯一性 */
                                id={name}
                            />
                        </div>
                    );
                ...
                case "UploadImageLB":
                    return (
                        <div className={clsx(uploadResultContainer, resultMaxHeightStyle, !showUploadResults && hideUploadResult)} key={`${forceUpdateFlag}-result`}>
                            <UploadResultLB
                                {...uploadResult as Omit<UploadResultLBProps, "initialFile">}
                                initialFile={(getFormValue([name], true)[name] ?? defaultVal) as UploadResultLBProps["initialFile"]}
                                onRemoveFile={onUploadRemove}
                                ref={uploadResultRef}
                                disabled={disabled}
                                /** 兼容formList情况上传组件相同id的情况，使用name能够保证id的唯一性 */
    ...
    return (
        <Stack direction={labelPosition} spacing={labelPosition === "row" && !hideLabel ? 16 : 8} grow="unset" width={wrapperWidth} basis="unset">
            {
                (labelPosition === "column" || !hideLabel || showStar) && (
                    <Stack direction="row" basis="unset" width={labelPosition === "row" && (label !== undefined || displayLabel) ? 88 : undefined} align="start" grow="unset">
                        <div className={clsx(labelBox, labelPosition === "row" && rowLabelBox)}>
                            <Label label={`${label ?? ""}`} required={showStar} width="100%" align={labelPosition === "row" ? "right" : "left"} info={info} />
                            {labelPosition === "column" && error !== undefined && error !== "" && <div className={clsx(errorBox)}>{error}</div>}
                        </div>
                    </Stack>
                )
            }
                    ...
                    : (
                        <Stack align="center" direction="row">
                            <div className={contentBox}>
                                {childrenFormItem()}
                                {uploadResults()}
                                {labelPosition === "row" && error !== undefined && error !== "" && <div className={clsx(errorBox)}>{error}</div>}
                            </div>
                        </Stack>
                    )
            }
